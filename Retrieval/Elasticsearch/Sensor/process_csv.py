...
ES_SENS_DATA_INDEX_NAME = 'sensordataindex'
ES_SENS_INDEX_MAPPING = {
    'mappings': {
        'properties': {
            'timestamp': {'type': 'date'},
            'value': {'type': 'float'},
...
def es_sens_create_index():
    es.indices.create(index=ES_SENS_DATA_INDEX_NAME, body=INDEX_MAPPING)
es_sens_create_index()

global id
id = 1

# Define a function to process a single CSV
def es_sens_process_csv(file_path, sensor_number, actions):
    global id
    ...
    for index, row in df.iterrows():
        action = {"index": {"_index": ES_SENS_DATA_INDEX_NAME, "_id": id}}
        doc = {
            "value": row['value'],
        ...
        id += 1
        if(id%100==0):
            es_sens_insert(actions)
            actions = []

    return actions

# Define a function to process multiple CSV files
def es_sens_process_multiple_csv(file_paths):
    
    for i, file_path in enumerate(file_paths):
        actions = []
        # Process current CSV file
        actions = es_sens_process_csv(file_path, i+1, actions)
        if(len(actions) != 0):
            es_sens_insert(actions)

def es_sens_insert(actions):
    # Insert data into Elasticsearch index
    es.bulk(index=ES_SENS_DATA_INDEX_NAME, operations=actions)

# Process each CSV file
file_paths = [f'DISPLACEMENT_WIRELESS#{j+1}_20171111061047.csv' for j in range(6)]

es_sens_process_multiple_csv(file_paths)
...
def es_sens_search(sensor_number, interval_start, interval_end):
    query = {
        "query": {
            "bool": {
                "must": [
                    {"range": {"timestamp": {"gte": interval_start, "lte": interval_end}}},
                    {"match": {"sensor_number": sensor_number}}
                ]
            }
        },
    ...
    result = es.search(index=ES_SENS_DATA_INDEX_NAME, body=query, size=10000)
    records = [hit['_source']['value'] for hit in result['hits']['hits']]
    return records

es_sens_trials = [[] for _ in range(6)]
...
for interval in intervals:
    ...
    es_sens_values = [[] for _ in range(6)]

    interval_start, interval_end = interval
    for sensor_number in range(1, 7):

        start_time = datetime.datetime.now()
        hits = es_sens_search(sensor_number, interval_start, interval_end)
        es_sens_values[sensor_number-1].extend(hits)
        end_time = datetime.datetime.now()
        es_sens_elapsed_time = (end_time - start_time).total_seconds() * 1000
        es_sens_trials[sensor_number-1].append(es_sens_elapsed_time)

for i in range(6):
    print("Sensor " + str(i+1))
    print("Time to Retrieve (milliseconds): " + str(np.average(es_sens_trials[i])))