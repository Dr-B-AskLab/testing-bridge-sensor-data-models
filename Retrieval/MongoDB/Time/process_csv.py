...
m_time_data_collection = db.create_collection('timedata')
m_time_hold_collection = db.create_collection('timehold')

global id
id = 1

# Define a function to process a single CSV file
def m_time_process_csv(file_path, sensor_number):
...
def m_time_process_multiple_csv(file_paths):
    df = m_time_process_csv(file_paths[0], 1)
    ...
    for i, file_path in enumerate(file_paths):
        ...
        temp_df = m_time_process_csv(file_path, i+2)
        df = df.join(temp_df.set_index(df.index))

    # Convert each row of DataFrame to a JSON object and insert into collection
    m_time_hold_collection.insert_many(df.to_dict('records'))

def m_time_insert(items):
    m_time_data_collection.insert_many(items)

# Process each CSV file
file_paths = [f'DISPLACEMENT_WIRELESS#{j+1}_20171111061047.csv' for j in range(6)]

m_time_process_multiple_csv(file_paths)
m_time_insert(m_time_hold_collection.find())
...
def es_time_search(sensor_number, interval_start, interval_end):
    ...
    result = es.search(index=ES_TIME_DATA_INDEX_NAME, body=query)
    values = []
    for hit in result['hits']['hits']:
        sensor_data = [hit['_source'][f'sensor_{i}'] for i in range(sensor_number, 0, -1)]
        values.append(sensor_data)
    values = np.array(values).T[sensor_number-1]
    return values

m_time_trials = [[] for _ in range(6)]
...
for interval in intervals:
    ...
    m_time_values = [[] for _ in range(6)]

    interval_start, interval_end = interval
    for sensor_number in range(1, 7):

        start_time = datetime.datetime.now()
        m_time_values[sensor_number-1].extend(m_time_search(sensor_number, interval_start, interval_end))
        end_time = datetime.datetime.now()
        m_time_elapsed_time = (end_time - start_time).total_seconds() * 1000
        m_time_trials[sensor_number-1].append(m_time_elapsed_time)


for i in range(6):
    print("Sensor " + str(i+1))
    print("Time to Retrieve (milliseconds): " + str(np.average(m_time_trials[i])))